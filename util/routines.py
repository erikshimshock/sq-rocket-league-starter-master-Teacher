...
# follow along from videos
class jumper():
    def run(self, agent):
        print("I want to jump", int(agent.time), int(agent.time % 5))

        if agent.time % 5 == 0: # this is what's in the video, but it's possible it never triggers
                                # can go from something like 4.97 to 0.02 and skip 0
            pass
        
        # fixed condition: needs casting to int to ignore the decimal time
        if int(agent.time % 5) == 0: 
...
class kickoff():
    ...
    def run(self, agent):
        # (shimshock comment)  ball is at (0,0,0), this aims for 200u in front of the ball
        #                             i.e (0, 200, 0) for orange or (0, -200, 0) for blue
        #   this seems to miss ball from one of the kickoff locations (TODO: FIX?)
